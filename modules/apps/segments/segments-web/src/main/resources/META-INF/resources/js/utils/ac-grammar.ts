 ...
 */
function buildDateQueryString(day: {operatorName: string; value: DateValue}) {
	let query = '';
...
function buildEventQueryString(
	...
	const [
		{assetId, day, operatorName, operatorNot, propertyName, value},
	] = criteria.items;
	...
	query = operatorNot ? `((not ${query}))` : `(${query})`;
