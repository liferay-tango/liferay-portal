		...
		<c:choose>
			<c:when test="<%= segmentsConfigurationURL != null %>">
				<clay:link
					...
					label='<%= LanguageUtil.get(request, "instance-settings-segments-segments-service.") %>'
				/>
			</c:when>
			<c:otherwise>
				<span><%= LanguageUtil.get(request, "instance-settings-segments-segments-service.") %></span>
			</c:otherwise>
		</c:choose>
	</clay:stripe>
</c:if>
