...
public class GetLayoutReportsIssuesMVCResourceCommand
	...
	protected void doServeResource(
		...
		try {
			...
			if (group == null) {
				_log.error("No API Key configured for the site " + groupId);

				JSONPortletResponseUtil.writeJSON(
					resourceRequest, resourceResponse,
					JSONUtil.put(
						"error",
						"problems retrieving the API Key for the site " +
							groupId));
			...
			List<LayoutReportsDataProvider.LayoutReportsIssue>
				layoutReportsIssues =
					layoutReportsDataProvider.getLayoutReportsIssues(
						canonicalURL);

			Stream<LayoutReportsDataProvider.LayoutReportsIssue> stream =
				layoutReportsIssues.stream();
